...
def save_history(history, prefix):
    ...
    if not os.path.exists(config.CLASSIFY['plots_dir']):
        os.mkdir(config.CLASSIFY['plots_dir'])

    img_path = os.path.join(config.CLASSIFY['plots_dir'], '{}-%s.jpg'.format(prefix))
...
def set_samples_info():
    ...
    dirs_info = {config.CLASSIFY['train_dir']: 0, config.CLASSIFY['validation_dir']: 0}
    for d in dirs_info:
    ...
    config.CLASSIFY['nb_train_samples'] = dirs_info[config.CLASSIFY['train_dir']]
    config.CLASSIFY['nb_validation_samples'] = dirs_info[config.CLASSIFY['validation_dir']]


def get_class_weight(d):
    white_list_formats = {'png', 'jpg', 'jpeg', 'bmp'}
    class_number = dict()
    dirs = sorted([o for o in os.listdir(d) if os.path.isdir(os.path.join(d, o))], reverse=True)
    k = 0
...
def set_classes_from_train_dir():
    ...
    d = config.CLASSIFY['train_dir']
    config.classes = sorted([o for o in os.listdir(d) if os.path.isdir(os.path.join(d, o))], reverse = True)
...
def get_classes_in_keras_format():
    if config.CLASSIFY['classes']:
        return dict(zip(config.CLASSIFY['classes'], range(len(config.CLASSIFY['classes']))))
    return None

#Import the 'model' from the folder: /classify/models/keras/
def get_model_class_instance(*args, **kwargs):
    module = importlib.import_module("classify.models.keras.{}".format(config.CLASSIFY['model']))
...
def save_activations(model, inputs, files, layer, batch_number):
    ...
    if batch_number > 0:
        submission.to_csv(config.CLASSIFY['activations_path'], index=False, mode='a', header=False)
    else:
        submission.to_csv(config.CLASSIFY['activations_path'], index=False)
...
def get_model_from_model_name():
    if config.CLASSIFY['model_name']:
        models_dir = os.path.join(
            config.CLASSIFY['trained_dir'],
            config.CLASSIFY['model_name'])

        # find trained-model (.h5-file) which lies in the
        os.chdir(models_dir)
        for filename in glob.glob("model-*.h5"):
            model = filename[6:-3]
            config.CLASSIFY['model'] = model
            logging.info("Automatically detected model architecture: {}".format(model))


def get_fine_tuned_weights_path(checkpoint=False):
    return os.path.join(
        get_named_model_path(),
        config.CLASSIFY['fine_tuned_weights_filename'].format(
            config.CLASSIFY['model'] + '-checkpoint' if checkpoint else config.CLASSIFY['model'])
    )
...
def get_classes_path():
    path = os.path.join(
        get_named_model_path(),
        config.CLASSIFY['classes_filename'].format(config.CLASSIFY['model'])
    )
    return path
...
def get_model_path():
    path = os.path.join(
        get_named_model_path(),
        config.CLASSIFY['models_filename'].format(config.CLASSIFY['model'])
    )
    return path


def get_named_model_path():
    return os.path.join(
        config.CLASSIFY['trained_dir'],
        config.CLASSIFY['model_name'])


def get_history_path(prefix=""):
    path = os.path.join(
        get_named_model_path(),
        prefix + "_" + config.CLASSIFY['history_filename']
    )
    return path